...
export default function MonthlySalary() {
  ...
  // =======================
  // EFFECT TO CHECK FOR UNPAID RECORD
  // =======================
  useEffect(() => {
  ...
  // =======================
  // CHECK FOR UNPAID RECORD BASED ON FILTERS
  // =======================
  const checkForUnpaidRecord = () => {
    const { teacherId, month, year } = filters;
    
    console.log("Checking for unpaid record with filters:", { teacherId, month, year });
    console.log("Available structures:", structures);
    
    if (!teacherId || !month || !year) {
      console.log("Missing filters, clearing selected record");
  ...
  const handleMarkAsPay = async () => {
    ...
    try {
      ...
      console.log("Calling mark as pay API with:", params);
      
      const res = await api.post(
        `/teacherSalary/pay?teacherId=${params.teacherId}&month=${params.month}&year=${params.year}`
      );
      
      console.log("Mark as Pay Response:", res.data);
      ...
      console.error("Mark as Pay Error:", err);
      ...
      <div className="monthly-wrapper">
        ...
        <div className="monthly-form-card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
            ...
            {/* MARK AS PAY BUTTON - Outside the form, positioned at top right */}
          ...
          {/* Alternative: Mark as Pay button below the form */}
          {selectedRecord && !isEditMode && (
